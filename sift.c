...
IplImage* create_init_img( IplImage*, int, double ,int  ,CvPoint ,CvPoint );
...
@see _sift_features()
*/
int sift_features( IplImage* img, struct feature** feat,int k, CvPoint pt1,CvPoint pt2)
{
	return _sift_features( img, feat, SIFT_INTVLS, SIFT_SIGMA, SIFT_CONTR_THR,
							SIFT_CURV_THR, SIFT_DESCR_WIDTH,
							SIFT_DESCR_HIST_BINS, k ,pt1,pt2);
}
...
int _sift_features( IplImage* img, struct feature** feat, int intvls,
				   ...
				   int descr_width, int descr_hist_bins,int k ,CvPoint pt1,CvPoint pt2)
{
	IplImage* init_img;
...
{
    octvs = log( MIN( img->width, img->height )*1.0 ) / log(2.0) - 3;
	octvs= octvs>3?3:octvs;
	img_dbl=1;
}
//printf("octave number =   %d \n", octvs ) ;

time_out = clock() ;
	init_img = create_init_img( img, img_dbl, sigma,k,pt1,pt2 );
//printf("create_init_img   %d ms\n", clock() - time_out ) ;

time_out = clock() ;
	gauss_pyr = build_gauss_pyr( init_img, octvs, intvls, sigma );
//printf("Gaussian 金字塔时间 %d ms\n", clock() - time_out ) ;
time_out = clock() ;
  	dog_pyr = build_dog_pyr( gauss_pyr, octvs, intvls );
//printf("DoG金字塔时间 %d ms\n", clock() - time_out ) ;
...
*/
IplImage* create_init_img( IplImage* img, int img_dbl, double sigma ,int k ,CvPoint pt1,CvPoint pt2)
{
	IplImage* gray, * dbl,*image,*img1;
	float sig_diff;
	int width, height,x,y,a,b;
	int step, step1,channels;
	uchar *data,*data1;
	...
	if(k==1)
	{
	   a =pt1.x-SIFT_IMG_BORDER;
	   b =pt1.y-SIFT_IMG_BORDER;
	   width = (pt2.x-pt1.x)+2*SIFT_IMG_BORDER;
	   height = (pt2.y-pt1.y)+2*SIFT_IMG_BORDER;
	   image = cvCreateImage( cvSize(width, height), 8, 3);
	   
	   step = img->widthStep/sizeof(uchar);
	   step1 = image->widthStep/sizeof(uchar);
	   channels =img->nChannels ;
	   data = (uchar *)img->imageData;
	   data1 = (uchar *)image->imageData;
	   for(y=0,b =pt1.y-SIFT_IMG_BORDER;y<height;y++,b++)
		 for(x=0,a =pt1.x-SIFT_IMG_BORDER;x<width;x++,a++)
		  {	
		     data1[y*step1+x*channels +0]  = data[b*step+a*channels +0];
			 data1[y*step1+x*channels +1]  = data[b*step+a*channels +1];
			 data1[y*step1+x*channels +2]  = data[b*step+a*channels +2];
		  }
      /*cvNamedWindow("image1",1);
	  cvShowImage("image1",image);
	  cvWaitKey(0);*/
      gray = convert_to_gray32( image );


	}
   if(k==2)
	{
       img1= cvCreateImage(cvSize(img->width, img->height), 8, 1) ;
	   cvCvtColor( img,img1, CV_BGR2GRAY ) ;

	   a =pt1.x-(pt2.x-pt1.x)-SIFT_IMG_BORDER;
	   b =pt1.y-(pt2.y-pt1.y)-SIFT_IMG_BORDER;
	   width = 3*(pt2.x-pt1.x)+2*SIFT_IMG_BORDER;
	   height = 3*(pt2.y-pt1.y)+2*SIFT_IMG_BORDER;
	   image = cvCreateImage( cvSize( width,height ), 8, 3);
	   ...
	   data = (uchar *)img->imageData;
	   data1 = (uchar *)image->imageData;
	   for(y=0, b =pt1.y-(pt2.y-pt1.y)-SIFT_IMG_BORDER;y<height;y++,b++)
		 for(x=0, a =pt1.x-(pt2.x-pt1.x)-SIFT_IMG_BORDER;x<width;x++,a++)
		  {	
		     data1[y*step1+x*channels +0]  = data[b*step+a*channels +0];
			 data1[y*step1+x*channels +1]  = data[b*step+a*channels +1];
			 data1[y*step1+x*channels +2]  = data[b*step+a*channels +2];
     ...
     /* cvNamedWindow("image",1);
	  ...
	  cvWaitKey(0);
	  cvSaveImage("E:\\capture\\image.jpg",image,0);
	  */
      gray = convert_to_gray32( image );
	}
	...
	if( img_dbl )
	{
		...
		dbl = cvCreateImage( cvSize( gray->width*2, gray->height*2 ),
			IPL_DEPTH_32F, 1 );
...
//			sprintf( chPath, "G:\\DoG\\DoG_%02d_%02d.bmp", o+1, i+1 ) ;
//			cvSaveImage( chPath, dog_pyr[o][i] ) ;  
		}
    ...
    for( i = 0; i < n; i++ )
	   {
		 ...
		 feat->x =(feat->x/2.0) ;
		 feat->y =(feat->y/2.0) ;
	//	 printf("x点的坐标x = %f,y点的坐标 y = %f\n",feat->x,feat->y);
		 feat->scl /= 2.0;
		 feat->img_pt.x = feat->img_pt.x /2.0 ;
		 feat->img_pt.y = feat->img_pt.y /2.0 ;
	   }	
}
...
void calc_feature_oris( CvSeq* features, IplImage*** gauss_pyr )
{
	...
	for( i = 0; i < n; i++ )
	{
		...
		ddata = feat_detection_data( feat );
//		printf("%.3f - %.0f\n", ddata->scl_octv, SIFT_ORI_RADIUS * ddata->scl_octv ) ;
